            ...
            with viz_col:
                try:
                    ...
                    if selected_type == "scatter":
                        if show_trend:
                            try:
                                fig = px.scatter(df_preview, x=selected_x, y=selected_y, color=color_arg, trendline="ols", title=default_title)
                            except Exception:
...
# Xử lý Chat mới
if prompt := st.chat_input("VD: Cho tôi insights về doanh thu và vẽ biểu đồ..."):
