...
# ==========================================
# 5. RENDER RESPONSE (POWER BI STYLE)
# ==========================================
def render_assistant_response(answer, audit_logs=None):
    answer = answer.strip()
    
    # Tạo widget_key độc nhất để tránh lỗi Duplicate Element ID
    ...
    # THỰC THI SQL ĐỂ LẤY DATA CHO POWER BI
    df_preview = None
    ...
    with tab1:
        ...
        # CHUYỂN ĐỔI SANG INTERACTIVE DASHBOARD
        if df_preview is not None and not df_preview.empty:
            ...
            # Ưu tiên gợi ý từ Agent nếu có
            if chart_spec and chart_spec.get("type") != "none":
                default_type = chart_spec.get("type", "bar")
                if chart_spec.get("x") in df_preview.columns: default_x = chart_spec["x"]
                if chart_spec.get("y") in df_preview.columns: default_y = chart_spec["y"]
                default_title = chart_spec.get("title", default_title)
            
            if default_type not in ["bar", "line", "pie", "scatter", "area"]: 
                default_type = "bar"
            ...
            with control_col:
                ...
                sel_type = st.selectbox("Loại biểu đồ", chart_types, index=chart_types.index(default_type), key=f"type_{widget_key}")
                sel_x = st.selectbox("Trục X (Dimension)", df_preview.columns, index=list(df_preview.columns).index(default_x), key=f"x_{widget_key}")
                sel_y = st.selectbox("Trục Y (Measure)", df_preview.columns, index=list(df_preview.columns).index(default_y), key=f"y_{widget_key}")
                color_choice = st.selectbox("Phân loại (Legend)", ["None"] + list(df_preview.columns), key=f"color_{widget_key}")
                show_trend = st.checkbox("📈 Bật đường dự báo (Trendline)", key=f"trend_{widget_key}")
            ...
            with viz_col:
                try:
                    ...
                    if sel_type == "scatter":
                        if show_trend:
                            try:
                                fig = px.scatter(df_preview, x=sel_x, y=sel_y, color=color_arg, trendline="ols", title=default_title)
                            except Exception:
                                fig = px.scatter(df_preview, x=sel_x, y=sel_y, color=color_arg, title=default_title)
                                st.warning("⚠️ Cần cài đặt thư viện 'statsmodels' (chạy lệnh: pip install statsmodels) để vẽ đường xu hướng OLS.")
                        else:
                            fig = px.scatter(df_preview, x=sel_x, y=sel_y, color=color_arg, title=default_title)
                    elif sel_type == "bar":
                        fig = px.bar(df_preview, x=sel_x, y=sel_y, color=color_arg, title=default_title)
                    elif sel_type == "line":
                        fig = px.line(df_preview, x=sel_x, y=sel_y, color=color_arg, title=default_title)
                    elif sel_type == "pie":
                        fig = px.pie(df_preview, names=sel_x, values=sel_y, title=default_title)
                    elif sel_type == "area":
                        fig = px.area(df_preview, x=sel_x, y=sel_y, color=color_arg, title=default_title)
                    
                    fig.update_layout(margin=dict(l=20, r=20, t=40, b=20), paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)")
                    st.plotly_chart(fig, use_container_width=True)
                except Exception as e:
                    st.warning(f"⚠️ Dữ liệu phân bổ không tương thích với loại biểu đồ này. Vui lòng đổi trục X/Y hoặc loại biểu đồ ở bảng điều khiển. (Lỗi: {e})")
